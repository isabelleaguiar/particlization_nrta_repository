...
void fig_10_get_histogram() {

    TFile *fout = new TFile("fig_10_histograms.root", "RECREATE");
    ...
    for (int i = 0; i < 3; i++) {
        for (int k = 0; k < 4; k++) { 
            ...
            std::string path = "./arquivos_root/no_decay/meanpT_dN_pPb_nodecay_" + gammas[k] + ".root";
