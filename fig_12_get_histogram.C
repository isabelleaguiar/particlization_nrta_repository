...
void fig_12_get_histogram() {
    
    TFile *fout = new TFile("fig_12_histograms.root", "RECREATE");
    ...
    for (int iSys = 0; iSys < 2; iSys++) {
        ...
        for (int iSpec = 0; iSpec < 4; iSpec++) {
            ...
            std::string fileNameRef = baseDir + "out_spectraOld_pPb_5p02_Duke_g0p5_nodecay_" + IDs[iSpec] + ".root";
            ...
            for (int iG = 1; iG < 4; iG++) {
                std::string fileName = baseDir + "out_spectraOld_pPb_5p02_Duke_" + gammas[iG] + "_nodecay_" + IDs[iSpec] + ".root";
