...
void fig_5_get_histogram() {
    
    TFile *fout = new TFile("fig_5_histograms.root", "RECREATE");

    std::string IDs[] = {"211", "321", "2212", "ch"}; 
    std::string species[] = {"Pion", "Kaon", "Proton", "Charged"};
    std::string gammas[] = {"g0", "g0p5", "g1", "g1p5"};
    std::string baseDir = "./arquivos_root/no_decay/";
    ...
    for (int iSys = 0; iSys < 2; iSys++) {
        ...
        for (int iSpec = 0; iSpec < 4; iSpec++) {
            ...
            std::string fileNameRef = baseDir + "out_spectraOld_PbPb_2p76_JETSCAPE_g0_1000_" + IDs[iSpec] + ".root";
            ...
            for (int iG = 1; iG < 4; iG++) {
                std::string fileName = baseDir + "out_spectraOld_PbPb_2p76_JETSCAPE_" + gammas[iG] + "_1000_" + IDs[iSpec] + ".root";
