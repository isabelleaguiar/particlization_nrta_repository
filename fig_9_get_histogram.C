...
void fig_9_get_histogram() {
    
    TFile *fout = new TFile("fig_9_histograms.root", "RECREATE");
    ...
    for (int iSys = 0; iSys < 2; iSys++) {
        ...
        for (int iSpec = 0; iSpec < 4; iSpec++) {
            ...
            std::string fileNameRef = baseDir + "out_spectraOld_PbPb_2p76_JETSCAPE_g0_usual_" + IDs[iSpec] + ".root";
            ...
            for (int iG = 1; iG < 4; iG++) {
                std::string fileName = baseDir + "out_spectraOld_PbPb_2p76_JETSCAPE_" + gammas[iG] + "_usual_" + IDs[iSpec] + ".root";
